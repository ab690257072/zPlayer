...
function showLyric() {
	var liH = $(".lyric li").eq(0).outerHeight(); //每行高度
	for (var i = 0; i < lyricArr.length; i++) {
		var curT = $(".lyric li").eq(i).attr("data-time"); //获取当前li存入的歌词时间
		var nexT = $(".lyric li").eq(i + 1).attr("data-time");
		var curTime = myAudio.currentTime;
		if ((curTime > curT) && (curT < nexT)) { //当前时间在下一句时间和歌曲当前时间之间时渲染并滚动
